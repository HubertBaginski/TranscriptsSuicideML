...
macro = macro %>% 
  mutate(Model = factor(Model, levels =c("Majority", "Tf-idf","BERT"), labels = c("Majority", "Tf-idf","BERT")), 
         ...
         Difficulty =  rep(rep(difficulty, each=3), 2),
         Model = fct_recode(Model, "Tf-idf & SVM" = "Tf-idf")) #repeat per model (3) and per validation/test set  
...
df_binary = ncat_plot %>% #ncat_propotions for proportion plot
  filter(!Classification.task %in% c("Problem vs Solution", "Main focus"), 
         dataset == "Train") %>% 
  droplevels() %>% 
...
#make sure this drops all factor levels of the level 4 classification tasks
binary = ggplot(df_binary, aes(x=Categories, y = n, fill=Categories, colour=Categories)) +
  geom_bar(stat="identity", position = position_dodge2(width = 0.8))+
  facet_wrap(~Classification.task, nrow=1)+
  scale_fill_manual(values=cols, name="")+
  scale_colour_manual(values=cols, name="")+
...
#make sure this drops all factor levels of the level 4 classification tasks
ps = ggplot(df_ps, aes(x=Categories, y = n, fill=Categories, colour = Categories)) +
  geom_bar(stat="identity", position = position_dodge2(width = 0.8))+
...
#make sure this drops all factor levels of the level 4 classification tasks
mf = ggplot(df_mf, aes(x=Categories, y = n, fill=Categories, colour = Categories)) +
  geom_bar(stat="identity", position = position_dodge2(width = 0.8))+
  facet_wrap(~Classification.task, nrow=2)+
  scale_fill_manual(values=cols, name="")+
  scale_colour_manual(values=cols, name="")+
...
combined
ggsave('./figures/Fig1_n_per_category_all_trainingset.tiff', combined, width=7, height=5, dpi=300)
```
...
plot = ggplot(filter(macro_plot, Dataset == "Test"), aes(x=Metric, y = Score, fill=Model, colour = Model)) + #dataset and variables to plot
  ...
  theme_bw() +  
  theme(legend.position="top", text=element_text(size=9), plot.margin = margin(rep(0.4,4), unit="cm"), axis.title.x = element_blank(), 
        axis.text.x = element_text(angle = 45, hjust=1)) #rotate x-axis tick labels
plot
ggsave('./figures/Fig2_Model_comparison_testset_permetric.tiff', plot, width=7, height=4.5, dpi=300)
```
...
tfidf = ggplot(filter(macro_plot, Model == "Tf-idf & SVM"), aes(x=Metric, y = Score, fill=Dataset, colour = Dataset)) + #dataset and variables to plot
  geom_bar(stat="identity", position = position_dodge2(width = 0.8))+
...
combined
ggsave('./figures/FigS1_test_vs_validation_performance.png', combined, width=7, height=8, dpi=300)
```
...
corrn = ggplot(intra_n)+
  geom_point(aes(x=n, y=Score, color = Categories), size=2.1)+
  facet_grid(Model~Metric)+
  theme_bw()+
  # scale_color_manual(values = c(rep(cols[1:4], times = c(1,1,4, 14))))+
  scale_color_manual(values = c("blue", "red3", "cyan", "gold", "olivedrab1", "darkgreen", 
                                      "grey", "purple4",  "#A6317DFF", "grey", "pink", rep("grey", 4), "violet", rep("grey", 4)))+
   scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.2)) +
   scale_x_continuous(limits = c(0, 1500), breaks = seq(0, 1500, 250)) +
  labs(x = "Class size n", y = "Performance")+
  theme(text=element_text(size=s+1), plot.margin = margin(rep(0.4,4), unit="cm"))
corrn
 ggsave('./figures/Fig4_correlation_n_performance_multicolors_trainingdataset.png', corrn, width=7.5, height=5.2, dpi=300)
```

```{r}
cor.test(~Score+n, data = filter(intra_n, Metric == "Precision" & Model =="Tf-idf & SVM"& n>50))
cor.test(~Score+n, data = filter(intra_n, Metric == "Recall" & Model =="Tf-idf & SVM" & n>50))

cor.test(~Scor<e+n, data = filter(intra_n, Metric == "Precision" & Model =="BERT" & n<50))
cor.test(~Score+n, data = filter(intra_n, Metric == "Recall" & Model =="BERT"))
```
