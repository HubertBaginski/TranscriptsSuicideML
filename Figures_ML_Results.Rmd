...
corrn = ggplot(intra_n)+
  ...
  # scale_color_manual(values = c(rep(cols[1:4], times = c(1,1,4, 14))))+
    scale_color_manual(values = c("cornflowerblue", "red4", rep("black", 4), rep("grey", 14)))+
  labs(y = "Class size n", x = "Performance")+
  theme(text=element_text(size=s+1), plot.margin = margin(rep(0.4,4), unit="cm"))
corrn
ggsave('./figures/Fig4_correlation_n_performance.pdf', corrn, width=7.5, height=5.2, dpi=300)
```
...
ggplot(intra_n)+
  geom_jitter(aes(x=Score, y=n, color = Categories, shape = Model), size= 2.5)+
  theme_bw()+
  scale_color_manual(values = c("blue2", "red4", rep("black", 4), rep("grey", 14)))+
  labs(y = "Class size n", x = "Performance")+
  theme(text=element_text(size=s+1), plot.margin = margin(rep(0.4,4), unit="cm"), legend.position = "bottom")
```
